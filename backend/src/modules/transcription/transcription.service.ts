// src/modules/transcription/azure-transcription.service.ts
import fs from 'fs';
import os from 'os';
...
import {
  ITranscriptionDocument,
  ITranscriptionLean,
} from './transcription.types';
import * as config from '../../config/env';

// ffmpeg-static provides an absolute path to a static ffmpeg binary per-platform
...
export class TranscriptionService {
  ...
  private async downloadToTemp(url: string): Promise<string> {
    ...
    const tmpName = `voiceowl_${Date.now()}_${Math.floor(
      Math.random() * 10000,
    )}`;

    // determine extension from URL; default to .mp3
    let ext = '.mp3';
    try {
      ...
      // fallback: try to parse ext from raw url
  ...
  /**
   * Convert input audio file to WAV PCM 16k mono using ffmpeg-static.
  ...
  private async convertToWavPcm16k(inputPath: string): Promise<string> {
    ...
    if (!ffmpegPath) {
      throw new Error(
        'ffmpeg-static did not provide a binary path. Install ffmpeg or ffmpeg-static.',
      );
    }
  ...
  async transcribeAzure(
    ...
    return retry(
      async () => {
        ...
        // 2) convert (ffmpeg-static)
        const wavPath = await this.convertToWavPcm16k(downloaded);

        // 3) call Azure Speech SDK
        const speechSdk = require('microsoft-cognitiveservices-speech-sdk');
        const fsPromises = fs.promises;
        const pushStream = speechSdk.AudioInputStream.createPushStream();

        // read file in manageable chunks to avoid huge memory usage for very large files
        ...
        // wrap SDK callback as Promise
        const result = await new Promise<{ text: string }>(
          (resolve, reject) => {
            // SDK timeout guard
            ...
            }, 60_000); // 60s recognition timeout (adjust if needed)
              ...
              (res: any) => {
                ...
                try {
                  ...
                  if (res.reason === speechSdk.ResultReason.RecognizedSpeech) {
                    ...
                    reject(
                      new Error(
                        `Speech SDK error: ${res.errorDetails || res.reason}`,
                      ),
                    );
                  }
                } finally {
